...
【テンポ】毎ターン同じ情報密度・同じ長さにしない。長い返しと短い返しを混ぜ、時には一言・一語で返す勇気を持つ。
...
作り方（いずれか）：
- 急な距離の詰め：フラットに話してたのに、核心を一言で突く（「それ、本当はこう思ってるでしょ」）。
- レジスターの落差：軽い口調の中に、一瞬だけ静かで鋭いトーンを差す。
- 見透かされ感：本人がまだ言葉にしていなかったことを、先回りして言い当てる。
原則：
- 相手が実際に話したことに根ざす。当てずっぽう・浅い材料で無理に刺さない。
...
export const MOVE_INSTRUCTION: Record<string, string> = {
  ...
  reflect: '評価も言い当てもせず、ごく短く受け止める（時に一言・一語で十分）。説明しすぎない。間を残す。',
